...
const container = cva("relative w-full min-w-[200px] h-10", {
  variants: {
    variant: {
      outline: "",
      static: "h-11",
...
interface ContainerVariants extends VariantProps<typeof container> {}
...
interface PlaceholderVariants extends VariantProps<typeof placeholder> {}

const input = cva("", {
  variants: {
    variant: {
      static:
        "peer h-full w-full border-b border-blue-gray-200 bg-transparent pt-4 pb-1.5 font-sans text-sm font-normal text-blue-gray-700 outline outline-0 transition-all placeholder-shown:border-blue-gray-200 focus:border-gray-900 focus:outline-0 disabled:border-0 disabled:bg-blue-gray-50",
      standard:
...
const Input = React.forwardRef<HTMLInputElement, InputProps>(
  ({ className, variant = "outline", label, ...props }, ref) => {
    return (
      <div className={cn(container({ variant }))}>
        ...
        />
        <label htmlFor={props.name} className={cn(placeholder({ variant }))}>
          {label}
        </label>
      </div>
    );
